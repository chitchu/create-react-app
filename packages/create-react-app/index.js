...
function createApp(name, verbose, version, template) {
  ...
  if (!isSafeToCreateProjectIn(root, validFiles)) {
  ...
  run(root, appName, version, verbose, originalDirectory, template, validFiles);
}
...
function run(root, appName, version, verbose, originalDirectory, template, validFiles) {
  ...
  install(packageToInstall, verbose, function(code, command, args) {
    if (code !== 0) {
      ...
      // Get a list of files from project folder.
      var directoryFiles = fs.readdirSync(path.join(originalDirectory, appName));
      // Diff it with the validFiles list
      var unlinkQueue = directoryFiles.filter(function (directoryFile) {
        return validFiles.indexOf(directoryFile) === -1;
      });

      if (unlinkQueue.length === directoryFiles.length) {
        // If the diff is similar to directory files
        // we can simply delete the folder.
        console.log();
        ...
        // Otherwise we individually delete
        // the files from the project folder
        console.log();
        console.log('Deleting created files... ');
        unlinkQueue.forEach(function (file) {
          console.log(chalk.cyan(file));
          fs.removeSync(path.join(originalDirectory, appName, file));
        });
      }
      console.log('Done.');
      process.exit(1);
...
function isSafeToCreateProjectIn(root, validFiles) {
